...
def search_products(query :str, max_price :Optional[float]= None , is_organic : Optional [bool] = None) -> str:
  ...
  0ptionally filter the maximum price and/or organic status. 
  Returns in JSON array of matching products, each with: id, name, category, price, 
...
def checkout(product_id : int) -> str:
   """
   Place an order for the given product ID. Saves the order ot the database and returns
...
@tool
def describe_prduct_image(image_path : str) -> str:
   """
...
agent = create_agent(
      tools = [search_products,get_rating,checkout,describe_prduct_image],
      model = llm,
      system_prompt = (
         ...
         "messages":[
            {
               "role": "user", 
               "content":(
                  "I want to buy organic tiger  with 4.5+ rating and less than $20 price. "
               )
            }
         ]
      }
